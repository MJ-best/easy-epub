...
class MarkdownParser {
  ...
  static String parseToHtml(
    String markdown, {
    MarkdownRenderOptions options = const MarkdownRenderOptions(),
  }) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (trimmed.startsWith('# ')) {
        ...
        closeListIfNeeded();
        buffer.writeln(
          '<h1${_classAttribute(['h1', ...options.styleClassHints.headingLevel1])}>${_escapeHtml(trimmed.substring(2))}</h1>',
        );
        continue;
      }
      ...
      if (trimmed.startsWith('## ')) {
        ...
        closeListIfNeeded();
        buffer.writeln(
          '<h2${_classAttribute(['h2', ...options.styleClassHints.headingLevel2])}>${_escapeHtml(trimmed.substring(3))}</h2>',
        );
        continue;
      }
      ...
      if (trimmed.startsWith('### ')) {
        ...
        closeListIfNeeded();
        buffer.writeln(
          '<h3${_classAttribute(['h3', ...options.styleClassHints.headingLevel3])}>${_escapeHtml(trimmed.substring(4))}</h3>',
        );
        continue;
      }
      ...
      if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
        ...
        if (!inList || currentListTag != 'ul') {
          ...
          buffer.writeln(
            '<ul${_classAttribute(['list', ...options.styleClassHints.unorderedList])}>',
          );
          inList = true;
          currentListTag = 'ul';
        }
        buffer.writeln(
            '<li>${_processInlineMarkdown(trimmed.substring(2))}</li>');
        continue;
      }
      ...
      if (RegExp(r'^\d+\.\s').hasMatch(trimmed)) {
        ...
        if (!inList || currentListTag != 'ol') {
          ...
          buffer.writeln(
            '<ol${_classAttribute(['list', ...options.styleClassHints.orderedList])}>',
          );
          inList = true;
          currentListTag = 'ol';
        }
      ...
      if (trimmed.startsWith('|') && trimmed.endsWith('|')) {
        ...
        if (isSeparator) {
          ...
          for (final cell in cells) {
            final tag =
                inTable && buffer.toString().contains('</thead>') ? 'td' : 'th';
            buffer.write(
                '<$tag style="border: 1px solid #ddd; padding: 8px;">${_processInlineMarkdown(cell.trim())}</$tag>');
          }
          buffer.writeln('</tr>');
        }
        continue;
      ...
      if (RegExp(r'^\!\[.*?\]\(.*?\)$').hasMatch(trimmed)) {
        ...
        if (match != null) {
          ...
          buffer.writeln(
              '<p class="txt bl center"><img src="${_escapeHtml(url)}" alt="${_escapeHtml(alt)}" style="max-width: 100%; height: auto;"/></p>');
        }
        continue;
      }
      ...
      if (trimmed.startsWith('> ')) {
        ...
        closeListIfNeeded();
        final quoteContent = trimmed.substring(2).trim();
        buffer.writeln(
          '<blockquote${_classAttribute(['quote', ...options.styleClassHints.blockquote])}>${_processInlineMarkdown(quoteContent)}</blockquote>',
        );
        continue;
      }

      // Raw HTML block passthrough for advanced EPUB styling
      if (_isRawHtmlLine(trimmed)) {
        if (inParagraph) {
          buffer.writeln('</p>');
          inParagraph = false;
        }
      ...
      if (!inParagraph) {
        buffer.write(
          '<p${_classAttribute(['txt', 'bl', ...options.styleClassHints.paragraph])}>',
        );
        inParagraph = true;
      }
  ...
  static String _processInlineMarkdown(String text) {
    ...
    result = result.replaceAllMapped(
      RegExp(r'!\[([^\]]*)\]\(([^\)]+)\)'),
      (match) =>
          '<img src="${_escapeHtml(match.group(2)!)}" alt="${_escapeHtml(match.group(1)!)}" style="max-width: 100%; height: auto;"/>',
    );
    ...
    result = result.replaceAllMapped(
      RegExp(r'\[(.+?)\]\((.+?)\)'),
      (match) =>
          '<a href="${_escapeHtml(match.group(2)!)}">${match.group(1)}</a>',
    );
  ...
  static bool _isRawHtmlLine(String line) {
    return RegExp(r'^<([a-zA-Z]+)(\s+[^>]*)?>.*</\1>$').hasMatch(line) ||
        RegExp(r'^<([a-zA-Z]+)(\s+[^>]*)?/?\s*>$').hasMatch(line);
  }

  static String _classAttribute(List<String> classes) {
