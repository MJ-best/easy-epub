...
void main() {
  group('미리보기 기능 테스트', () {
    testWidgets('미리보기가 마크다운을 HTML로 변환', (tester) async {
      final repository = _FakeEbookRepository();
      final epubGenerator = EpubGeneratorServiceV2();
      final viewModel = CreateBookViewModel(repository, epubGenerator);
      ...
      // 본문 입력 (마크다운 포함)
      final contentField = find.widgetWithText(TextField, '마크다운으로 작성하세요\n\n# 제목\n## 부제목\n본문 내용...');
      await tester.enterText(
        contentField,
        '# 제목 1\n\n본문 내용입니다.\n\n**굵은 글씨**와 *기울임*\n\n- 목록1\n- 목록2',
      );
      ...
      // 미리보기에 콘텐츠가 표시되는지 확인
      expect(viewModel.content.isNotEmpty, true);
      expect(viewModel.content.contains('# 제목 1'), true);
      expect(viewModel.content.contains('**굵은 글씨**'), true);
    });

    testWidgets('템플릿 선택이 미리보기에 반영됨', (tester) async {
      final repository = _FakeEbookRepository();
      final epubGenerator = EpubGeneratorServiceV2();
      final viewModel = CreateBookViewModel(repository, epubGenerator);
    ...
    testWidgets('표 마크다운이 미리보기에서 렌더링됨', (tester) async {
      final repository = _FakeEbookRepository();
      final epubGenerator = EpubGeneratorServiceV2();
      final viewModel = CreateBookViewModel(repository, epubGenerator);
      ...
      expect(viewModel.content.contains('|'), true);
    });

    testWidgets('이미지 마크다운이 포함됨', (tester) async {
      final repository = _FakeEbookRepository();
      final epubGenerator = EpubGeneratorServiceV2();
      final viewModel = CreateBookViewModel(repository, epubGenerator);
    ...
    testWidgets('편집 도구 버튼이 마크다운 문법을 삽입함', (tester) async {
      final repository = _FakeEbookRepository();
      final epubGenerator = EpubGeneratorServiceV2();
      final viewModel = CreateBookViewModel(repository, epubGenerator);
